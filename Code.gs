...
function handleGetDashboard(params) {
  ...
  const userId = params ? (params.user_id || params.judge_id) : null;
  let currentUser = users.find(function(u) { return u.id == userId; });

  let votedCountForJudge = 0;
  const processedContestants = contestants.map(function(c) {
    let voted = false;
    let scoreInfo = { total_score: 0 };
    if (userId) {
      const s = scores.find(function(item) { return item.contestant_id == c.id && item.judge_id == userId; });
      if (s) {
        voted = true;
        votedCountForJudge++;
        scoreInfo = { total_score: Number(s.total_score) };
      }
    }
    return {
      id: Number(c.id),
  ...
  const result = {
    judge_id: currentUser ? currentUser.id : null,
    judge_name: currentUser ? currentUser.name : (users[0] ? users[0].name : 'กรรมการ'),
    round: activeRound,
    ...
    total_contestants: processedContestants.length,
    voted_count: votedCountForJudge,
    remaining_count: processedContestants.length - votedCountForJudge,
...
function handleSubmitVote(data) {
  ...
  const judgeId = data.judge_id || (data.user ? data.user.id : 1);
  const contestantId = data.contestant_id;
  const roundId = data.round_id;

  const scores = getSheetData(SHEET_NAMES.SCORES);
  
  // Find if score row already exists for this judge, contestant & round
  let existingRowIdx = -1;
  let existingScoreId = -1;
  for (let i = 1; i < scores.length; i++) {
    if (scores[i].judge_id == judgeId && scores[i].contestant_id == contestantId && scores[i].round_id == roundId) {
      existingRowIdx = i + 1;
      existingScoreId = scores[i].id;
      break;
    }
  }

  // Calculate total_score and details if scores dict was passed
  let totalScore = Number(data.total_score || 0);
  let details = data.details || [];

  if ((!details || details.length === 0) && data.scores) {
    totalScore = 0;
    details = [];
    for (let cId in data.scores) {
      let val = Number(data.scores[cId]) || 0;
      totalScore += val;
      details.push({ criterion_id: Number(cId), score: val });
    }
  }
  ...
  if (existingRowIdx > 0) {
    ...
    scoresSheet.getRange(existingRowIdx, 5, 1, 2).setValues([[totalScore, now]]);
    
    // Replace details
    const allDetails = detailsSheet.getDataRange().getValues();
    for (let j = allDetails.length - 1; j >= 1; j--) {
      if (allDetails[j][1] == existingScoreId) {
        detailsSheet.deleteRow(j + 1);
      }
    }
    let lastRow = detailsSheet.getLastRow();
    details.forEach(function(d) {
      lastRow++;
      detailsSheet.appendRow([lastRow, existingScoreId, d.criterion_id, d.score]);
    });

    invalidateCache();
    logAuditInSheet(ss, data.judge_name || 'Judge', judgeId, 'UPDATE_VOTE', `Updated score ${totalScore} for contestant #${contestantId}`);
    return { status: 'success', message: 'บันทึกแก้ไขคะแนนเรียบร้อยแล้ว', score_id: existingScoreId };
  } else {
    // Create new score row
    const newScoreId = scoresSheet.getLastRow() > 1 ? (Number(scoresSheet.getRange(scoresSheet.getLastRow(), 1).getValue()) + 1) : 1;
    scoresSheet.appendRow([newScoreId, judgeId, contestantId, roundId, totalScore, now]);

    let detailsCount = detailsSheet.getLastRow();
    details.forEach(function(d) {
      detailsCount++;
      detailsSheet.appendRow([detailsCount, newScoreId, d.criterion_id, d.score]);
    });

    invalidateCache();
    logAuditInSheet(ss, data.judge_name || 'Judge', judgeId, 'SUBMIT_VOTE', `Submitted score ${totalScore} for contestant #${contestantId}`);
    return { status: 'success', message: 'บันทึกคะแนนเรียบร้อยแล้ว', score_id: newScoreId };
  }
}
